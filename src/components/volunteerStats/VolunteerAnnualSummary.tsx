...
import { ANNUAL_ROUTE } from "../../routes/route.constants";
import { VolunteerCardsProps } from "./types";

import styles from "./styles/volunteerStats.module.css";

export const VolunteerAnnualSummary = ({ simpleMode }: VolunteerCardsProps) => {
  const navigate = useNavigateHook();
  const handleClick = () => {
    navigate(ANNUAL_ROUTE);
  };
    ...
    <>
      {simpleMode ? (
        <div className={styles.simpleVolunteerCard}>
          <span className={styles.volunteerStatsTitle}>Annual Summary</span>
