...
begin
  ...
  select exists (
    ...
    where roster.roster_owner_id = target_signup.event_owner_id
  ...
  if acting_user_id <> target_signup.event_owner_id
    and acting_user_id <> target_signup.event_coordinator_id
  ...
  set
    status = case
      when target_signup.status = 'no_show'
        and updated_check_in_at is not null
        and updated_check_out_at is not null
      then 'confirmed'
      else status
    end,
    volunteer_start_time = nullif(trim(volunteer_start_time_value), ''),
    volunteer_end_time = nullif(trim(volunteer_end_time_value), ''),
    volunteer_location = volunteer_location_value,
    volunteer_impact = volunteer_impact_value,
    check_in_at = updated_check_in_at,
    check_out_at = updated_check_out_at
  where id = target_signup_id;
end;
$$;

grant execute on function public.update_event_signup_impact_details(uuid, text, text, text, text) to authenticated;
